...
TString TimeSmearOption = "MC"; // "MC" or "Gaus"
TRandom3 randgen;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     if(UseTrueEnergy) { 
       ...
       hEnergy_vacuum->Fill(tvac.arrival_e*1000); // convert to keV  	
     	 ...
     	 double tof_arg = targ.arrival_time/1000;
     	 double tof_vac = tvac.arrival_time/1000;
     	 if(TimeSmearOption == "MC") {
       ...
       else if(TimeSmearOption == "Gaus") {
       	 // use Gaussian moderator response 
       	 double tof_arg_smear = randgen.Gaus(0, T0Res/1000/2.35); // FWHM -> sigma
     	   double tof_vac_smear = randgen.Gaus(0, T0Res/1000/2.35); // FWHM -> sigma
     	   tof_arg += tof_arg_smear;
         tof_vac += tof_vac_smear;
       }
     	 
     	 // add detector time resolution
     	 tof_arg += randgen.Gaus(0, TDetRes/1000);
     	 tof_vac += randgen.Gaus(0, TDetRes/1000);
