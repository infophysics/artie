{
  ...
  TFile f1("Artie_argon_fill_test.root");      
...
/*
  ...
  for(int i=1; i<=1000; i++){
    BinEntry1[1] = hist1->GetBinContent(i);
    BinEntry2[1] = hist2->GetBinContent(i);
    //produced 1,000,000 neutrons with uniform random distribution
    //There are 1000 bins
    //So there are 1000 neutrons in each energy bin
  ...
  auto legend1 = new TLegend(0.7,0.7,0.9,0.9);
   ...
   legend1->AddEntry(h1,"Aluminium","l");
   legend1->AddEntry(h3,"Stainless Steel","l");  
   legend1->Draw();

  c2->cd();
  auto legend2 = new TLegend(0.7,0.7,0.9,0.9);
   legend2->SetHeader("Legend","C"); // option "C" allows to center the header
   legend2->AddEntry(h2,"Aluminium","l");
   legend2->AddEntry(h4,"Stainless Steel","l");  
   legend2->Draw();
*/
}