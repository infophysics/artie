...
void PrimaryGenerator::UseUserDefinedEnergy(std::string inputfile, std::string objectname)
{ 
    ...
    fEnergyFile = new TFile(inputfile.c_str());
    //fEnergy = (TH1D*)fEnergyFile->Get(objectname);
    TGraph *gr = (TGraph*)fEnergyFile->Get(objectname.c_str());
